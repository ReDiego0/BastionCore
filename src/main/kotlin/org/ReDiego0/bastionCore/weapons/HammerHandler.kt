...
class HammerHandler(private val plugin: BastionCore) {
    ...
    fun handleRightClick(player: Player) {
        ...
        player.velocity = player.location.direction.multiply(1.0).setY(1.2) // Salto medio
        player.playSound(player.location, Sound.ENTITY_IRON_GOLEM_ATTACK, 1f, 0.5f)

        plugin.server.scheduler.runTaskLater(plugin, Runnable {
            if (!player.isOnline) return@Runnable
            player.world.spawnParticle(Particle.BLOCK_CRUMBLE, player.location, 50, 1.0, 0.1, 1.0, 0.1, org.bukkit.Material.DIRT.createBlockData())
